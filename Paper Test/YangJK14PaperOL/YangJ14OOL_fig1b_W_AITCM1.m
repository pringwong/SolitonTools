...
Lx=20;Nx=800; maxerror=1e-10; nmax=5000;k0=2*pi/Lx;
...
%     c
%     flag=0;
% for DT=0.1:0.5:5.1;%默认值c=3.8,DT=0.7
%      if flag==1
%          break;
%      end
     for P=2.1:-0.05:1.1
 ...
 u=sech(x).*exp(x);  
%  2*exp(-(x+3).^2)-4*exp(-(x-2).^2)+3*exp(-(x+1).^2);                 % initial condition
%tanh(x)
...
%      save result.txt sa -ascii -append
   figure(1)
   plot(real(mu),P,'.','color',[col1,col2,col3]),hold on
%    ,axis([4.4 5.2 0 4])
   else
       flag=1;
%           break;
